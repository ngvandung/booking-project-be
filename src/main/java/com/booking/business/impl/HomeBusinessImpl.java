...
public class HomeBusinessImpl implements HomeBusiness {
	...
	public Home updateHome(long homeId, String name, long homeTypeId, String typeName, long stateId, String stateName,
		...
		if (PermissionCheckerFactoryUtil.isOwner(userContext, home.getUserId())) {
			return homeService.updateHome(homeId, name, homeTypeId, typeName, stateId, stateName, cityId, cityName,
					...
					isActive, ownerHomeId, userContext.getUser().getUserId());
		} else {
			throw new ForbiddenException();
		}
	}
	...
	public Home createHome(String name, long homeTypeId, String typeName, long stateId, String stateName, long cityId,
		...
		return homeService.createHome(name, homeTypeId, typeName, stateId, stateName, cityId, cityName, districtId,
				...
				isFridge, isBalcony, isWindows, isSmartTv, isExtraMattress, description, ownerHomeId,
	...
	public List<Map<String, Object>> findMyHomes(Long ownerHomeId, String flag, UserContext userContext) {
		if (PermissionCheckerFactoryUtil.isOwner(userContext, ownerHomeId)) {
			return homeService.findMyHomes(ownerHomeId, flag);
		} else {
			throw new ForbiddenException();
		}
	}
